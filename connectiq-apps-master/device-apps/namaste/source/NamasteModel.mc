...
class NamasteModel
{
    ...
    function initialize() {
        ...
        mSession = ActivityRecording.createSession({:sport=>Activity.SPORT_GENERIC, :name=>"Yoga"});
    ...
    function accelCallback() as Void {
        var info = Sensor.getInfo();
        //System.println("info=" + info + "info has :accel=" + info has :accel);

        // Capture the accel vector
        if( info has :accel) {
            // info.accel may be null although info has :accel is true !!!!
            mSamples[mSampleCounter] = info.accel ? info.accel : [1,1,1];
        } else {
        ...
        if (mSampleCounter < 3) {
            ...
            a = mSamples[0];
            //System.println("a " + a + ", mSamples=" + mSamples + ", mSampleCounter=" + mSampleCounter);
